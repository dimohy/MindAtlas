...
### 1.1 솔루션 스캐폴딩
- [ ] `MindAtlas.slnx` 솔루션 생성 (.NET 9 slnx 형식)
- [ ] `src/MindAtlas.Core/` 클래스 라이브러리 프로젝트 생성
- [ ] `src/MindAtlas.Engine/` 클래스 라이브러리 프로젝트 생성
- [ ] `src/MindAtlas.Server/` ASP.NET Core 프로젝트 템플릿 생성
- [ ] `src/MindAtlas.Web/` Blazor WASM 프로젝트 템플릿 생성
- [ ] `src/MindAtlas.Desktop/` Avalonia 프로젝트 템플릿 생성
- [ ] `tests/MindAtlas.Engine.Tests/` xUnit 프로젝트 생성
- [ ] `tests/MindAtlas.Server.Tests/` xUnit 프로젝트 생성
- [ ] 프로젝트 간 참조 설정 (Core → Engine → Server, Core → Web 등)
- [ ] `schema/AGENTS.md` 기본 Schema 템플릿 생성
- [ ] `.gitignore`, `Directory.Build.props` 등 공통 설정

### 1.2 Core 도메인 모델
- [ ] `WikiPage` 모델 (Title, Summary, Content, Tags, WikiLinks, CreatedAt, UpdatedAt)
- [ ] `RawSource` 모델 (FileName, FilePath, ContentType, AddedAt, Status)
- [ ] `IndexEntry` 모델 (PageName, Summary, Tags, Keywords)
- [ ] `LogEntry` 모델 (Timestamp, Operation, Description, AffectedPages)
- [ ] `LintResult` 모델 (OrphanPages, BrokenLinks, Conflicts, MissingIndex)
- [ ] `QueryResult` 모델 (Answer, SourcePages, NewInsights)
- [ ] `VibeCodingAsset` 모델 (AssetType enum, Name, Content, Tags)
- [ ] `IWikiRepository` 인터페이스 (위키 CRUD)
- [ ] `IRawRepository` 인터페이스 (raw 파일 관리)
- [ ] `IWikiEngine` 인터페이스 (Ingest/Query/Lint)
- [ ] `IIndexService` 인터페이스 (빠른 검색)

### 1.3 1단계 검증
- [ ] `dotnet build MindAtlas.slnx` 성공, 0 에러
- [ ] 7개 프로젝트 모두 참조 해결 완료
...
- [ ] `WikiRepository` 구현 — wiki/ 디렉토리 CRUD (마크다운 읽기/쓰기)
- [ ] `RawRepository` 구현 — raw/ 디렉토리 파일 관리
- [ ] index.md 파싱/업데이트 로직
- [ ] log.md append-only 기록 로직

### 2.2 LLM Wiki Engine — Ingest
- [ ] `IngestPipeline` 클래스 구현
  - [ ] raw 파일 읽기 (텍스트/마크다운)
  - [ ] GitHub Copilot SDK Agent로 내용 분석 및 위키 페이지 생성 요청
  - [ ] Agent 응답 파싱 → wiki/ 파일 생성/업데이트
  - [ ] `[[wikilinks]]` 교차참조 자동 삽입
  - [ ] `index.md` 업데이트
  - [ ] `log.md`에 Ingest 활동 기록
- [ ] 이미지/바이너리 파일 처리 (메타데이터만 위키에 기록)
- [ ] 기존 위키 페이지와 병합 로직

### 2.3 LLM Wiki Engine — Query
- [ ] `QueryEngine` 클래스 구현
  - [ ] Fast Path: index.md 키워드 매칭
  - [ ] AI Path: Agent에 위키 컨텍스트 + 질문 전달
  - [ ] 답변에서 새 인사이트 추출 → 위키 갱신
  - [ ] `log.md`에 Query 활동 기록

### 2.4 LLM Wiki Engine — Lint
- [ ] `LintEngine` 클래스 구현
  - [ ] 고아 페이지 탐지 (아무데서도 `[[link]]` 안 되는 페이지)
  - [ ] 깨진 wikilink 탐지 (존재하지 않는 페이지 링크)
  - [ ] `index.md` 불일치 탐지 (등록 안 된 페이지, 삭제된 페이지)
  - [ ] 구조화된 `LintResult` 반환

### 2.5 GitHub Copilot SDK Agent 연동
- [ ] Copilot SDK NuGet 패키지 추가
- [ ] `CopilotAgentService` 클래스 — Agent 생성/관리
- [ ] Schema(AGENTS.md) 내용을 Agent 시스템 프롬프트에 주입
- [ ] wiki/ 컨텍스트를 Agent에 전달하는 전략 구현
- [ ] Agent 응답 스트리밍 처리

### 2.6 Fast Index
- [ ] `IndexService` 클래스 구현
  - [ ] `index.md` 파싱 → `IndexEntry` 목록
  - [ ] wiki/ 파일 H1, wikilink 메타데이터 추출
  - [ ] 인메모리 인덱스 구축 (Dictionary/Trie 기반)
  - [ ] 키워드 검색 (접두사/부분 매치, < 50ms)
  - [ ] 파일 변경 시 인덱스 자동 리로드

### 2.7 FileSystemWatcher
- [ ] `RawDirectoryWatcher` 클래스 구현
  - [ ] `raw/` 디렉토리 감시 (Created, Changed 이벤트)
  - [ ] 디바운싱 (짧은 시간 내 다수 이벤트 병합)
  - [ ] Ingest 파이프라인 트리거
  - [ ] 처리 상태 추적 (대기→처리중→완료/실패)

### 2.8 2단계 검증
- [ ] 단위 테스트: IndexService 키워드 검색 < 50ms (1,000 항목)
- [ ] 단위 테스트: LintEngine 고아/깨진링크 탐지
- [ ] 단위 테스트: WikiRepository 파일 CRUD
- [ ] 단위 테스트: IngestPipeline 응답 파싱 (마커/폴백/위키링크/태그)
- [ ] 통합 테스트: raw/에 파일 추가 → wiki/ 페이지 자동 생성 (3단계 이후 실행)
- [ ] 통합 테스트: Query 엔진 Fast Path + AI Path (3단계 이후 실행)
- [ ] `dotnet test` 전체 통과 — 24 tests, 0 failures
...
### 3.6 3단계 검증
- [ ] OpenAPI 엔드포인트 등록 확인 (`/openapi/v1.json`)
...
- [ ] `dotnet build` 7개 프로젝트 성공, `dotnet test` 25 tests 통과
...
### 4.1 MCP 프로토콜 기반
- [ ] MCP JSON-RPC 메시지 핸들러 구현 (ModelContextProtocol SDK 사용)
- [ ] `initialize` / `initialized` 핸드셰이크 (SDK 내장)
- [ ] `tools/list` — 사용 가능한 도구 목록 반환 (SDK 내장)
- [ ] `tools/call` — 도구 실행 디스패쳐 (SDK 내장)

### 4.2 stdio 전송
- [ ] stdin/stdout 기반 JSON-RPC 리더/라이터 (WithStdioServerTransport)
- [ ] MCP 서버 프로세스 엔트리포인트 (`--mcp-stdio` CLI 인자)

### 4.3 SSE 전송
- [ ] ASP.NET Core HTTP MCP 엔드포인트 (`/mcp`, MapMcp)
- [ ] Streamable HTTP 전송 (ModelContextProtocol.AspNetCore 내장)
- [ ] 세션 관리 (SDK 내장)
...
- [ ] 라우팅 설정 (/, /wiki/{name}, /search, /query, /log)
- [ ] 공통 레이아웃 (사이드바 + 메인 콘텐츠)

### 5.2 위키 목록/조회
- [ ] `WikiList` (Home) 페이지 — index 기반 카드 뷰
- [ ] `WikiView` 페이지 — Markdig 마크다운 렌더링
- [ ] `[[wikilinks]]` 클릭 → 내부 네비게이션
- [ ] Breadcrumb 네비게이션

### 5.3 검색
- [ ] 검색 바 (Search 페이지)
- [ ] 실시간 키워드 필터링 (300ms 디바운싱)
- [ ] 검색 결과 카드 뷰

### 5.4 AI Query
- [ ] Query 입력 UI (채팅형 인터페이스)
